...
const userMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case LOG_IN: {
      ...
      axios.post('http://34.192.216.20/api/login_check', {
        username,
        password,
      })
        .then((response) => {
    ...
    case ALERT_CHANGE: {
      ...
      axios.post(`http://34.192.216.20/api/user/edit/alertday/${alertLevel}`, {
        alertLevel,
      }, {
    ...
    case HANDLE_REGISTRATION: {
      ...
      axios.post('http://34.192.216.20/api/login/signon', {
