...
import {
  styler, tween, merge, action, easing, keyframes,
} from 'popmotion';
...
class KawaiiAnimation extends Component {
  constructor(props) {
    ...
    this.chocoRef = React.createRef();
  }

  componentDidMount() {
    const mouth = styler(
      this.chocoRef.current.querySelector('#Combined-Shape'),
    );
    const tongue = styler(
      this.chocoRef.current.querySelector('#kawaii-face__tongue'),
    );
    const eyeLeft = styler(
      this.chocoRef.current.querySelector(
    ...
    const body = styler(this.chocoRef.current.querySelector('svg'));
  ...
  render() {
    return (
      <div ref={this.chocoRef}>
        <Chocolate className="kawaii_large" color="#684FA3" />
